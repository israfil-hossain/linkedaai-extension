...
const API_BASE_URL = "https://outlye.flowtim.com";
...
document.addEventListener("DOMContentLoaded", async () => {
  console.log("🚀 Outlye initialized");
...
function setupEventListeners() {
  // Delegated handler for external links (replaces inline onclick="event.stopPropagation()")
  document.addEventListener("click", (e) => {
    if (e.target.closest(".ext-link")) e.stopPropagation();
  }, true);
...
function setupMessageGeneration() {
  // Tone selector
  ...
  if (toneSelector) {
    toneSelector.addEventListener("click", (e) => {
      ...
      selectedTone = btn.dataset.tone || "professional";
    });
  }

  // Generate button
  $("generate-btn")?.addEventListener("click", handleGenerateMessage);

  // Copy button
  $("copy-message-btn")?.addEventListener("click", () => {
    if (generatedMessage) {
  ...
  // Insert into LinkedIn button
  $("insert-message-btn")?.addEventListener("click", async () => {
    ...
    if (result.success) {
      ...
      showToast("Open a LinkedIn message box first", "error");
    }
  });
  ...
  chrome.runtime.onMessage.addListener((message) => {
    if (message.type === "STREAM_UPDATE") {
      ...
      if (outputSection && outputBox) {
        outputSection.style.display = "block";
        outputBox.classList.add("streaming");
        outputBox.innerHTML = escapeHtml(message.fullMessage) + '<span class="cursor-blink"></span>';
        generatedMessage = message.fullMessage;
      }
    }
  });
}
...
function handleGenerateMessage() {
  ...
  if (generateBtn) {
    ...
    generateBtn.innerHTML = '<span class="button-spinner"></span> Generating...';
  }

  if (outputSection) outputSection.style.display = "block";
  if (outputBox) {
    outputBox.classList.add("streaming");
    outputBox.innerHTML = '<span class="output-placeholder">Generating...</span>';
  }
  ...
  sendBg({
    ...
    if (result.success && result.message) {
      ...
      if (outputBox) {
        ...
        outputBox.innerHTML = escapeHtml(result.message);
      }
      // Save message with the lead
      if (state.profile) {
        sendBg({
          type: "SAVE_LEAD",
          payload: {
      ...
      if (outputBox) {
        outputBox.classList.remove("streaming");
        outputBox.innerHTML = `<span class="output-placeholder" style="color: var(--danger);">${escapeHtml(result.error || "Generation failed")}</span>`;
      }
    ...
    if (outputBox) {
      outputBox.classList.remove("streaming");
      outputBox.innerHTML = '<span class="output-placeholder" style="color: var(--danger);">Network error</span>';
    }
...
function handleGoogleSearch() {
  ...
  resultsDiv.innerHTML = `
    ...
    <div style="font-size: 11px; color: #333;">
      <a href="https://www.google.com/search?q=${encodeURIComponent(query)}" target="_blank" class="ext-link" style="color: #3B82F6; text-decoration: none; display: block; margin: 4px 0;">
        🔍 Google Search
      </a>
      <a href="https://www.linkedin.com/search/results/people/?keywords=${encodeURIComponent(name + " " + company)}" target="_blank" class="ext-link" style="color: #0077b5; text-decoration: none; display: block; margin: 4px 0;">
        💼 LinkedIn Search
      </a>
    </div>
  `;
...
function renderNoteCardContent(content) {
  ...
  if (url) {
    return `<a href="${escapeHtml(url)}" target="_blank" rel="noreferrer noopener" class="note-content-link ext-link">${escapeHtml(trimmed)}</a>`;
  }
